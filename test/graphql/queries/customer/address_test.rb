...
class Queries::AddressTest < ActionDispatch::IntegrationTest
  ...
  test 'should retrieve own record as customer' do
    ...
    post graphql_path, params: { query: fetch_address_helper(user.addresses.first.id) }

    assert_response :success
    assert_equal user.addresses.first.id, json['data']['fetchAddress']['id'].to_i

    post graphql_path, params: { query: fetch_address_helper(users(:one).addresses.last.id) }
